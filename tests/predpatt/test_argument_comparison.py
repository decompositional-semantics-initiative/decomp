...
import pytest
from decomp.semantics.predpatt.patt import (
    Token as OriginalToken,
    Argument as OriginalArgument,
    sort_by_position as orig_sort_by_position
)
from decomp.semantics.predpatt.core import (
    Token as ModernToken,
    Argument as ModernArgument,
    sort_by_position as mod_sort_by_position
)
...
class TestArgumentComparison:
    ...
    def test_mutable_default_rules(self):
        """Test that mutable default rules behaves the same."""
        ...
        # both should show the quirk - shared default list
        # The key is that both implementations behave the same way
        assert "test_mutable" in orig2.rules
    ...
    def test_coords_identical(self):
        ...
        assert isinstance(orig_coords[1].rules[0], R.m)
        assert isinstance(modern_coords[1].rules[0], R.m)
